...
const ProfilePage = ({ stylist, profile }) => {
  ...
  const sortStylist = async (text) => {
    ...
    try {
      const res = await axios.post(
        `https://cosmotologywm.herokuapp.com/api/v1/UserRoute/sort`,
        {
          text,
        }
      );
  ...
  const sortClient = async (text) => {
    console.log(`Here is the text: ${text}`);
    try {
      const res = await axios.post(`https://cosmotologywm.herokuapp.com/api/v1/UserRoute/sort2`, {
        text,
      });
...
ProfilePage.getInitialProps = async (ctx) => {
  try {
    ...
    const res = await axios.get(
      `https://cosmotologywm.herokuapp.com/api/v1/profile/${userId}`,
      {
